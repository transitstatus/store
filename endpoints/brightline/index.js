...
const diffBetweenArrivalAndDeparture = { MCO: 0, WPT: 120, RRN: 120, FBT: 120, AVE: 120, EKW: 0 };
...
const updateFeed = async () => {
  ...
  let finalBrightlineV1 = {
    ...
    shitsFucked: { shitIsFucked: false, message: "" }
  };
  let finalBrightlineV2 = {
    ...
    routes: {},
    alerts: {},
    meta: { lastUpdated: nowDate.toISOString(), error: { shitIsFucked: false, message: "" } }
  };
  let platformsData = {};
  ...
  try {
    ...
    const processRequests = {
      brightlineTimes: (r) =>
        r.arrayBuffer().then((buf) => GtfsRealtimeBindings.transit_realtime.FeedMessage.decode(new Uint8Array(buf))),
      brightlinePositions: (r) =>
        r.arrayBuffer().then((buf) => GtfsRealtimeBindings.transit_realtime.FeedMessage.decode(new Uint8Array(buf))),
      brightlineStops: (r) => r.json(),
      brightlineRoutes: (r) => r.json(),
      brightlineVehicleSchedule: (r) =>
        r.arrayBuffer().then((buf) => MultipleVehiclesScheduleMessage.decode(new Uint8Array(buf))),
      brightlineScheduleMetadata: (r) => r.json()
    };

    // fetching all of our data asyncronously
    const fetchedDataRes = await Promise.all(
      Object.keys(urlsToFetch).map((key) => fetch(urlsToFetch[key] + `?currentTime=${Date.now()}`))
    );
    ...
    fetchedData.brightlineScheduleMetadata.stoppingPatterns.forEach((pattern, patternIndex) => {
      ...
      for (let i = 1; i < pattern.length; i++)
        totalTime += fetchedData.brightlineScheduleMetadata.stopTimes[`${pattern[i - 1]}_${pattern[i]}`];
    ...
    const platformsRes = await Promise.all(
      stationsInOrder.map((stop) =>
        fetch(
          `https://schedules.gobrightline.com/api/schedulefordisplay?stationName=${stop.stopName}&DeparturesOrArrivals=Departures`
        )
      )
    );
    ...
    fetchedData.brightlineTimes.entity.forEach((trip) => {
      ...
      const position = vehiclePositions[trip.id] ?? { latitude: 0, longitude: 0, bearing: 0, fake: true };

      //doing some magic to make up for a not found position
      if (position.fake) {
        const upcomingStops = trip.tripUpdate.stopTimeUpdate.filter(
          (stopTime) => (stopTime.departure ?? stopTime.arrival).time.toNumber() > nowDate.valueOf() / 1000
        );
    ...
    const fillInVehicleData = (vehicleSchedule, now, todayStart) => {
      ...
      upcomingVehiclesWithinTimeFrame.forEach((vehicle) => {
        ...
        fetchedData.brightlineScheduleMetadata.stoppingPatterns[vehicle.vehicleStop].forEach((stop, i, arr) => {
          finalScheduledVehicle.predictions.push({
            //adding prediction
            stationID: stop,
          ...
          if (i == arr.length - 1) {
            //dont have to do the next step if this is the last station
            finalScheduledVehicle.dest = fetchedData.brightlineStops[stop].stopName;
          } else {
            //moving time to next station
    ...
    finalBrightlineV1.shitsFucked.message =
      "There was an error fetching from Brightline's API, please try again later.";
    return { v1: finalBrightlineV1, platforms: platformsData };
  }
};
